...
@section('title', 'Редактирование ' . $edit->name)

@section('content')
	<form action="{{route('updateAnimeAdmin', $edit->id)}}" class="bg-dark p-3" method="post" multiple enctype="multipart/form-data">
		@csrf
		<div class="input-group mb-3">
			<label for="name" class="input-group-text">Заголовок</label>
			<input type="text" name="name" id="name" class="form-control" placeholder="Заголовок" aria-label="Заголовок" aria-describedby="name" value="{{$edit->name}}">
		...
		<div class="row mb-3">
			<div class="col-6">
				<div class="input-group">
					...
					<input type="text" id="license_name_ru" class="form-control" name="license_name_ru" value="{{$edit->license_name_ru}}">
				</div>
			</div>
			<div class="col-6">
				<div class="input-group">
					<label for="english" class="input-group-text">Английское название</label>
					<input type="text" id="english" class="form-control" name="english" value="{{$edit->english}}">
				</div>
			</div>
		</div>
		<div class="row mb-3">
			<div class="col-6">
				<div class="input-group">
					...
					<input type="text" id="japanese" class="form-control" name="japanese" value="{{$edit->japanese}}">
				</div>
			</div>
			<div class="col-6">
				<div class="input-group mb-3">
					<label for="synonyms" class="input-group-text">Альтернативные название</label>
					<input type="text" id="synonyms" class="form-control" name="synonyms" value="{{$edit->synonyms}}">
				</div>
			</div>
		</div>
		<div class="row mb-3">
			<div class="col-4">
				...
				<select class="js-selectize-multiple" multiple aria-label="category" name="category[]" id="category">
					@foreach($category as $value)
						<option value="{{$value->id}}" @if($edit->getCategory->contains($value->id)) selected @endif>{{$value->title}}</option>
					@endforeach
				</select>
			</div>
			<div class="col-4">
				...
				<select class="js-selectize" aria-label="kind_id" name="kind_id" id="kind_id">
					@foreach($kind as $value)
						<option value="{{$value->id}}" @if($edit->getKind->id == $value->id) selected @endif>{{$value->full_name}}</option>
					@endforeach
				</select>
			</div>
			<div class="col-4">
				...
				<select class="js-selectize" aria-label="channel_id" name="channel_id" id="channel_id">
					@foreach($channel as $value)
						@if ($edit->getChannel)
							<option value="{{$value->id}}" @if($edit->getChannel->id == $value->id) selected @endif>{{$value->title}}</option>
						@else
			...
			<div class="col-3">
				...
				<select class="js-selectize-multiple" multiple aria-label="country" name="country[]" id="country">
					@foreach($country as $value)
						@if ($edit->getCountry)
							<option value="{{$value->id}}" @if($edit->getCountry->contains($value->id) == $value->id) selected @endif>{{$value->title}}</option>
						@else
							<option value="0">нет</option>
						@endif
			...
			<div class="col-3">
				...
				<select class="js-selectize-multiple" multiple aria-label="studios" name="studios[]" id="studios">
					@foreach($studios as $value)
						@if ($edit->getStudio)
							<option value="{{$value->id}}" @if($edit->getStudio->contains($value->id) == $value->id) selected @endif>{{$value->title}}</option>
						@else
							<option value="0">нет</option>
						@endif
						<option value="{{$value->id}}">{{$value->title}}</option>
			...
			<div class="col-3">
				...
				<select class="js-selectize-multiple" multiple aria-label="quality" name="quality[]" id="quality">
					@foreach($quality as $value)
						@if ($edit->getQuality)
							<option value="{{$value->id}}" @if($edit->getQuality->contains($value->id) == $value->id) selected @endif>{{$value->title}}</option>
						@else
							<option value="0">нет</option>
						@endif
						<option value="{{$value->id}}">{{$value->title}}</option>
			...
			<div class="col-3">
				...
				<select class="js-selectize-multiple" multiple aria-label="rating" name="rating[]" id="rating">
					@foreach($mpaa as $value)
						@if ($edit->getRating)
							<option value="{{$value->id}}" @if($edit->getRating->id == $value->id) selected @endif>{{$value->name}}</option>
						@else
							<option value="0">нет</option>
						@endif
						<option value="{{$value->id}}">{{$value->name}}</option>
		...
		<div class="row mb-3">
			<div class="col-3">
				<div class="input-group mb-3">
					...
					<input type="time" id="broadcast" class="form-control" name="broadcast" value="{{$edit->broadcast}}">
				</div>
			</div>
			<div class="col-3">
				<div class="input-group mb-3">
					<label for="episodes" class="input-group-text">Количество эпизодов</label>
					<input type="number" id="episodes" class="form-control" name="episodes" value="{{$edit->episodes}}">
				</div>
			</div>
			<div class="col-3">
				<div class="input-group mb-3">
					<label for="duration" class="input-group-text">Длительность (мин)</label>
					<input type="number" id="duration" class="form-control" name="duration" value="{{$edit->duration}}">
				</div>
			</div>
			<div class="col-3 row">
				<div class="col-6">
					<div class="form-check form-switch">
						<input name="anons" class="form-check-input" type="checkbox" id="anons" value="1" {{$edit->anons ? 'checked' : ''}}>
						<label class="form-check-label" for="anons">Анонс</label>
					</div>
				</div>
				<div class="col-6">
					<div class="form-check form-switch">
						<input name="ongoing" class="form-check-input" type="checkbox" id="ongoing" value="1" {{$edit->ongoing ? 'checked' : ''}}>
		...
		<div class="row">
			<div class="col-6">
				<div class="input-group mb-3">
					...
					<input type="date" id="aired_on" class="form-control" name="aired_on" value="{{$edit->aired_on}}">
				</div>
			</div>
			<div class="col-6">
				<div class="input-group mb-3">
					<label for="released_on" class="input-group-text">Дата окончания показа</label>
					<input type="date" id="released_on" class="form-control" name="released_on" value="{{$edit->released_on}}">
				</div>
			</div>
		</div>
		<div class="mb-3">
			...
			<input type="hidden" id="original_img" name="original_img" value="{{$edit->original_img}}">
		</div>
		...
		<div class="row">
			<div class="mb-3">
				...
				@foreach($edit->getTrailer as $trailer)
		...
		<div class="row">
			<div class="mb-3">
				...
				@foreach($edit->getPlayer as $player)
					<div class="row">
						<div class="col-2">
		...
		<div class="row">
			<div class="mb-3">
				...
				@foreach($edit->getOtherLink as $otherLink)
					<div class="row">
						<div class="col-2">
			...
			<div class="mb-3">
				...
				<select class="js-selectize-multiple" multiple aria-label="translate" name="translate[]" id="translate">
					@foreach($translate as $value)
						<option value="{{$value->id}}" @if($edit->getTranslate->contains($value->id)) selected @endif>{{$value->title}}</option>
					@endforeach
				</select>
			</div>
		</div>
		<div class="row mb-3">
			<div class="col-6">
				<div class="input-group">
					...
					<input type="date" id="updated_at" class="form-control" name="updated_at" value="{{\Carbon\Carbon::parse($edit->updated_at)->format('Y-m-d')}}">
				</div>
			</div>
			<div class="col-6">
				<div class="input-group">
					<label for="created_at" class="input-group-text">Дата добавления</label>
					<input type="date" id="created_at" class="form-control" name="created_at" value="{{\Carbon\Carbon::parse($edit->created_at)->format('Y-m-d')}}" disabled>
				</div>
			</div>
		</div>
		<div class="row mb-3">
			...
			<select class="js-selectize" aria-label="user_id" name="user_id" id="user_id">
				@foreach($user as $value)
					<option value="{{$value->id}}" @if($edit->getUser->id == $value->id) selected @endif>{{$value->login}}</option>
				@endforeach
			</select>
		</div>
		<div class="row mb-3">
			<div class="col-6">
				...
				<select class="js-selectize-multiple" aria-label="region" name="region[]" id="region" multiple>
					@foreach($geoBlock as $value)
						<option value="{{$value->id}}" @if ($edit->getRegionBlock->contains($value->id)) selected @endif >{{$value->country}}</option>
					@endforeach
				</select>
			</div>
			<div class="col-6">
				...
				<select class="js-selectize-multiple" aria-label="copyright_holder" name="copyright_holder[]" id="copyright_holder" multiple>
					@foreach($copyrightHolder as $value)
						<option value="{{$value->id}}" @if ($edit->getCopyrightHolder->contains($value->id)) selected @endif >{{$value->title}}</option>
					@endforeach
				</select>
			</div>
		</div>
		<div class="row mb-3">
			...
			<input id="reason_edit" name="reason_edit" type="text" class="form-control" value="{{$edit->reason_edit}}">
		</div>
		<div class="row">
			<div class="col-4">
				<div class="form-check form-switch">
			...
			<div class="col-4">
				<div class="form-check form-switch">
					<input name="posted_rss" class="form-check-input" type="checkbox" id="posted_rss" value="1" {{$edit->posted_rss ? 'checked' : ''}}>
					<label class="form-check-label" for="posted_rss">Выводить в Rss</label>
				</div>
			</div>
			<div class="col-4">
				<div class="form-check form-switch">
					<input name="comment_at" class="form-check-input" type="checkbox" id="comment_at" value="1" {{$edit->comment_at ? 'checked' : ''}}>
